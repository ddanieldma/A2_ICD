...
df_countries_cardio_2019 = df_cause_of_death[df_cause_of_death["Year"] == 2019].groupby(['Country/Territory', 'Continente']).agg({'Cardiovascular Diseases': 'sum'}).reset_index()
...
qs = df_continents_boxplot.groupby("kind")["Cardiovascular Diseases"].quantile([0.25, 0.5, 0.75])
...
# agrupando por continentens
df_continents_grouped = df_continents_boxplot.groupby("kind").sum().reset_index()

# criando column data source
source_continents_boxplot = ColumnDataSource(df_continents_grouped)
...
outliers = df_continents_grouped[~df_continents_grouped["Cardiovascular Diseases"].between(df_continents_grouped.lower, df_continents_grouped.upper)]